...
{   
    ...
    auto thread_task = [&](size_t start_p1, size_t end_p1, LocalHistograms* hists) {
        for (size_t p1 = start_p1; p1 < end_p1; p1++) {
            for (size_t p2 = p1 + 1; p2 < n_tracks; p2++) {
                if (currentEv.charges[p1] * currentEv.charges[p2] > 0) {
                    ...
                    if (dEta < 0.1 && dPhi < 0.1) {
                        hists->hSigSS->Fill(dEta, dPhi, currentEv.weights[p1] * currentEv.weights[p2]);
                    }
                }
            }
        }
        ...
        for (size_t p1 = start_p1; p1 < end_p1; ++p1) {
            for (const auto& pastEv : zBucketPool) {
                for (size_t p2 = 0; p2 < pastEv.tracks.size(); ++p2) {
                    if (currentEv.charges[p1] * pastEv.charges[p2] > 0) { 
                        ...
                        if (dEta < 0.1 && dPhi < 0.1) {
                            hists->mixSS->Fill(dEta, dPhi, currentEv.weights[p1] * pastEv.weights[p2]);
                        }
                    }
                }
            }
