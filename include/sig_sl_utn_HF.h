#ifndef SIG_SL_UTN_HF_H
#define SIG_SL_UTN_HF_H
...
#include "data_func.h"
#include "my_func.h"
#include "sig_sl_utn_HF.h"

void sig_sl_utn_HF(const char *fileInput, const char *treeInput, int selectionVarI, int poolSize = 10) {
    ...
    double nnscale = 10000, x0 = 0., xt=100.;
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        ...
        // === SIGNAL ===
        int s = currentEventTracks4V.size();
        if (currentEventTracks4V.size() > 1) { // check if event has 2 or more tracks 
            int npairs = (s * (s - 1)) / 2;
            // Single Loop
            for (int w = 0; w < npairs; w++) {
                // Analytically calculate i and j
                int p1 = s - 2 - static_cast<int>(std::floor(std::sqrt(-8 * w + 4 * s * (s - 1) - 7) / 2.0 - 0.5));
                int base = (p1 * (2 * s - p1 - 1)) / 2;
                int offset = w - base;
                int p2 = p1 + 1 + offset;
    ...
    // Prefix
    char prefix[50]; 
    sprintf(prefix, "sig_sl_utn_%s_%d-%d", selectionVarName, selectionVarI, selectionVarI);
