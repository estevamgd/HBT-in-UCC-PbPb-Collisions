...
void doubleRatioFitMix(ControlVar selectedControlVar,
                    double bin_low, double bin_high,
                    Double_t q1, Double_t q2,
                    qMode mode,
                    double fitMin = 0.0, double fitMax = 10.0, double fitMinBG = 0.15,
                    ...
                    const char* fileName = nullptr)
{
    ...
    const char* name_ssHist = Form("h_%sSSCor_signal_2l", qmodename);
    const char* name_mixHist = Form("h_%sCor_mix_2l", qmodename);
    ...
    // ===== Single ratio ===== //
    TH1D* singleRatio = histhistRatio(
        q_ssHist_cor, q_mixHist_cor, q1, q2, "sr_cor"
    );

    saveRatio(singleRatio, "sr_cor", plotXMin, plotXMax, 
        "sr_cor", "PbPb 2.76 TeV | Single Ratio",
        (mode == qMode::QLCMS)
            ? "; q_{LCMS} [GeV]; C(q_{LCMS}) = SS/Mix"
            : "; q_{inv} [GeV]; C(q_{inv}) = SS/Mix",
        "Single Ratio (SS/Mix)"
    );
...
int main() {
    ...
    // --- Analysis
    //doubleRatioFitMix(selectedControlVar, bin_low, bin_high, q1, q2, modeLCMS, fitMin, fitMax, fitMinBg, plotXMin, plotXMax);
    //doubleRatioFitMix(selectedControlVar, bin_low, bin_high, q1, q2, modeQinv, fitMin, fitMax, fitMinBg, plotXMin, plotXMax);
