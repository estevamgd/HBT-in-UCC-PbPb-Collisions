...
>>>>>>> 2b091b9659e95d3fd75285d7dfdd974665166055
    ...
    // 1. Define the search pattern for the input file
    // This pattern matches files generated by the `sig_double_loop` functions
    ...
    // --- Analysis Set 1: Corrected Ratio (SSCor / OSCor) ---
    std::cout << "\n--- Running Analysis 1: Corrected Ratio ---" << std::endl;
    
    // 2. Define histogram names for corrected ratio
    const char* ssHist_cor = "h_qinvSSCor_signal_2l"; // Corrected Same-Sign pairs
    const char* osHist_cor = "h_qinvOSCor_signal_2l"; // Corrected Opposite-Sign pairs

    // 3. Define output prefix and histogram name for corrected ratio
    TString outputPrefix_cor = TString::Format(
        "SingleRatio_Cor_%s_%.0f-%.0f", 
        selectionVarName, bin_low, bin_high
    );
    const char* outputHist_cor = "sr_cor"; // "sr_cor" stands for "single ratio corrected"
    
    // 4. Define plot header text for corrected ratio
    TString plotHeader_cor = TString::Format(
        "PbPb 2.76 TeV | %s %.0f-%.0f (Cor)",
        selectionVarName, bin_low, bin_high
    );

    // --- Run the Macro for Corrected Ratio ---
...
>>>>>>> 2b091b9659e95d3fd75285d7dfdd974665166055
    ...
    // 2. Define histogram names for uncorrected ratio
    const char* ssHist_uncor = "h_qinvSS_signal_2l"; // Uncorrected Same-Sign pairs
    const char* osHist_uncor = "h_qinvOS_signal_2l"; // Uncorrected Opposite-Sign pairs

    // 3. Define output prefix and histogram name for uncorrected ratio
    TString outputPrefix_uncor = TString::Format(
        "SingleRatio_Uncor_%s_%.0f-%.0f", 
        selectionVarName, bin_low, bin_high
    );
    const char* outputHist_uncor = "sr_uncor"; // "sr_uncor" stands for "single ratio uncorrected"
    
    // 4. Define plot header text for uncorrected ratio
    TString plotHeader_uncor = TString::Format(
        "PbPb 2.76 TeV | %s %.0f-%.0f (Uncor)",
        selectionVarName, bin_low, bin_high
    );

    // --- Run the Macro for Uncorrected Ratio ---
